...
pub struct HumanDevelopmentComponent {
    ...
    /// Последнее известное значение `DivisionExhaustionState::asymmetric_count`.
    /// Используется для детектирования новых асимметричных делений (Δcount → D-IDI release).
    pub last_asymmetric_count: u32,
...
impl SimulationModule for HumanDevelopmentModule {
    ...
    fn step(&mut self, world: &mut World, dt: f64) -> SimulationResult<()> {
        ...
        for (_, (comp, inflammaging_opt, exhaustion_opt, centriole_opt, mut telomere_opt, mut epigenetic_opt, mut diff_status_opt, mut modulation_opt, mito_opt)) in query.iter() {
            ...
            // Детектирование новых асимметричных делений для D-IDI высвобождения.
            // asymmetric_count монотонно растёт; разность = число новых делений за шаг.
            let current_asymmetric_count = exhaustion_opt.map_or(0, |e| e.asymmetric_count);
            let new_asymmetric_divisions = current_asymmetric_count
                .saturating_sub(comp.last_asymmetric_count);
            ...
            if self.params.enable_aging {
                ...
                // 4в. D-IDI функциональное высвобождение при асимметричном делении.
                // Детерминированный путь: одно деление → структурный переход дочь→мать
                // при синтезе прокентриоли внучатой клетки → высвобождение одного D-IDI.
                // Вызывается по одному разу на каждое новое асимметричное деление.
                if new_asymmetric_divisions > 0 {
                    for _ in 0..new_asymmetric_divisions {
                        detach_d_idi_by_division(&mut comp.inducers, &mut self.rng);
                    }
                }
                ...
                if let Some(ref mut epi) = epigenetic_opt {
                    ...
                    // Верхняя граница 1.30: эпигенетические часы ускоряются максимум на 30%
                    // (согласуется с данными Horvath 2013 — патологическое ускорение ≤30%).
                    epi.clock_acceleration = (1.0 + damage * 0.5).min(1.30);
            ...
            // Обновить счётчик делений — независимо от enable_aging,
            // чтобы не накапливать backlog при повторном включении.
            comp.last_asymmetric_count = current_asymmetric_count;
            }
        }
        ...
        //   - В фертильной фазе: ros_level снижается до 20% (гормональная антиоксидантная защита)
        //   - После пубертата: base_detach_probability × life_history_factor (≤1.20)
        //     (ресурсы «перенаправляются» с репарации центриолей на репродукцию)
        //   - Life-History Trade-off: ранний пубертат → бо́льший factor → быстрее стареют
        //     Источник: Tkemaladze J. "Theory of Lifespan Decline" (r=0.78, R²=0.92)
        {
            for (_, (dev, horm)) in world.query_mut::<(
                &mut HumanDevelopmentComponent,
                &mut HormonalFertilityState,
            )>() {
                if !dev.is_alive { continue; }
                let age = dev.age_years() as f32;
                hormonal_fertility::update_hormonal_fertility(horm, age);

                // Гормональная защита: снижаем ros_level (вычитаем ros_reduction)
                let ros_mod = hormonal_fertility::hormonal_ros_modifier(horm);
                if ros_mod.abs() > 1e-6 {
                    dev.centriolar_damage.ros_level =
                        (dev.centriolar_damage.ros_level + ros_mod).clamp(0.0, 1.0);
                    dev.centriolar_damage.update_functional_metrics();
                }

                // Life-History Trade-off: умножаем вероятность потери индукторов
                let lhf = hormonal_fertility::life_history_detach_multiplier(horm);
                if lhf > 1.001 {
                    let p = &mut dev.inducers.detachment_params.base_detach_probability;
                    *p = (*p * lhf).min(0.01);
                }
            }
        }
...
mod inductor_calibration_tests {
    ...
    /// Простейшая проверка: за 78 лет M-IDI теряется хотя бы один раз.
    ///
    /// Тест работает без CentrioleModule и MitochondrialModule — только fallback O₂-путь.
    /// При base_detach=0.0003 M-комплект ожидаемо теряет ~1 индуктор за 78 лет.
    /// D-комплект без AsymmetricDivisionModule не изменяется (только делительный путь).
    /// Проверку по нескольким семенам — см. `test_inductor_calibration_multiseed`.
    #[test]
    fn test_inductor_depletion_occurs() {
        ...
        // D-IDI: без делительных событий не должен меняться
        assert_eq!(d_rem, d_init,
            "D-IDI без делений должен оставаться полным: D={}/{}", d_rem, d_init);
        // M-IDI: O₂ должен разрушить хотя бы 1 индуктор за 78 лет
        assert!(m_loss >= 1,
            "M-IDI должен потерять хотя бы 1 индуктор за 78 лет: M={}/{}", m_rem, m_init);
    }

    /// Многосемянная калибровка (исправленная теория CDATA):
    ///
    /// - M-IDI (мать): O₂ разрушает → ≥0.5 потери за 78 лет (среднее по 5 запускам)
    /// - D-IDI (дочь): высвобождается ТОЛЬКО при делении (detach_d_idi_by_division).
    ///   Этот тест не регистрирует AsymmetricDivisionModule → нет делительных событий →
    ///   D-комплект ДОЛЖЕН оставаться полным (проверяем, что O₂ не трогает D).
    ///
    ...
    fn test_inductor_calibration_multiseed() {
        ...
        // M-IDI: O₂ разрушает → должна быть заметная потеря
        assert!(m_avg_loss >= 0.5,
            "M-комплект: средняя потеря за 78 лет должна быть ≥0.5 индуктора: {:.2}", m_avg_loss);

        // D-IDI: только делительный путь, AsymmetricDivisionModule не подключён →
        // нет делительных событий → D-комплект неизменен
        assert_eq!(d_avg_loss, 0.0,
            "D-комплект: без делительных событий O₂ не должен снимать D-IDI, \
             но детектирована потеря: {:.2}", d_avg_loss);
    }
}
