    ...
    fn step(&mut self, world: &mut World, dt: f64) -> SimulationResult<()> {
        ...
        for (_, (transcriptome, cell_cycle, centriole_opt, gene_expr_opt)) in query.iter() {
            ...
            if let Some(gene_expr) = gene_expr_opt {
                ...
                // ── Cyclin E синтез/деградация белка (T1) ──────────────────────
                // Синтез: mRNA CCNE1 × MYC-усиление (MYC активирует CCNE1-промотор)
                // Деградация: SCF-Fbxw7 убиквитин-лигаза — активна в S/G2/M,
                //   обеспечивает клиренс Cyclin E после инициации S-фазы
                //   (предотвращает повторную репликацию ДНК).
                // В G1 SCF почти неактивен → Cyclin E накапливается,
                // в S активируется → Cyclin E быстро деградирует.
                let ccne1_mrna = transcriptome.genes.get("CCNE1")
                    .map(|g| g.expression_level).unwrap_or(0.0);
                let myc_boost = 1.0 + gene_expr.myc_level * 0.3;
                let synthesis = ccne1_mrna * myc_boost * 0.25;
                let scf_activity: f32 = match cell_cycle.phase {
                    Phase::G1 => 0.15, // Cyclin E накапливается
                    Phase::S  => 0.60, // клиренс после инициации S
                    Phase::G2 => 0.70, // максимальный клиренс
                    Phase::M  => 0.50, // умеренный клиренс
                };
                let degradation = scf_activity * gene_expr.cyclin_e_level;
                gene_expr.cyclin_e_level = (gene_expr.cyclin_e_level
                    + (synthesis - degradation) * dt_f32)
                    .clamp(0.0, 1.0);
            }
        }
