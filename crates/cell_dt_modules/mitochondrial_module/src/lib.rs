...
impl SimulationModule for MitochondrialModule {
    ...
    fn step(&mut self, world: &mut World, dt_days: f64) -> SimulationResult<()> {
        ...
        {
            // --- Биологический возраст (прокси) ---
            // total_damage_score() используется как прокси хронологического возраста:
            // он монотонно растёт с возрастом и отражает накопленные молекулярные повреждения.
            // Это не настоящий возраст в годах — это нормализованный показатель повреждённости.
            // Порог 0.25 эмпирически соответствует ~40 годам (калибровка: Normal aging preset).
            let bio_age_proxy = damage.total_damage_score();

            // --- Мультипликатор среднего возраста ---
            // Антагонистическая плейотропия: после bio_age_proxy > 0.25 (~40 лет) скорость мутаций растёт
...
mod tests {
    ...
    fn test_pericentriolar_density_contributes_to_shield() {
        ...
        // Шаг 1: «здоровая» сеть — высокий fusion, нулевые мтДНК мутации
        ...
        // → pericentriolar_density = 0.9×0.70 + (1-0.025)×0.30 ≈ 0.92
        {
            ...
            state.mtdna_mutations    = 0.0;
            state.membrane_potential = 0.9;
        }
        ...
        assert!(healthy_shield > aged_shield,
            "mito_shield_contribution должен быть выше при здоровой сети: {:.3} vs {:.3}",
            healthy_shield, aged_shield);

        // Значения в допустимом диапазоне
        assert!(healthy_density >= 0.0 && healthy_density <= 1.0, "density out of [0,1]: {}", healthy_density);
        assert!(aged_density    >= 0.0 && aged_density    <= 1.0, "density out of [0,1]: {}", aged_density);
