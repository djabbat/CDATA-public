...
impl SimulationManager {
    ...
    /// Проверяет, что зависимые модули зарегистрированы в правильном порядке.
    /// Вызывается автоматически в `initialize()`.
    fn validate_module_order(&self) -> SimulationResult<()> {
        let names: Vec<&str> = self.modules.iter().map(|(n, _)| n.as_str()).collect();
        let pos = |name: &str| names.iter().position(|&n| n == name);

        // HumanDevelopmentModule синхронизирует CentriolarDamageState —
        // должен идти ДО MyeloidShiftModule, который эти данные читает.
        if let (Some(hd), Some(ms)) = (pos("human_development_module"), pos("myeloid_shift_module")) {
            if hd > ms {
                return Err(SimulationError::ModuleError(
                    "human_development_module must be registered before myeloid_shift_module".into()
                ));
            }
        }

        // CentrioleModule накапливает PTM — должен идти ДО HumanDevelopmentModule.
        if let (Some(cm), Some(hd)) = (pos("centriole_module"), pos("human_development_module")) {
            if cm > hd {
                return Err(SimulationError::ModuleError(
                    "centriole_module must be registered before human_development_module".into()
                ));
            }
        }

        // MitochondrialModule вычисляет mito_shield — должен идти ДО HumanDevelopmentModule.
        if let (Some(mm), Some(hd)) = (pos("mitochondrial_module"), pos("human_development_module")) {
            if mm > hd {
                return Err(SimulationError::ModuleError(
                    "mitochondrial_module must be registered before human_development_module".into()
                ));
            }
        }
