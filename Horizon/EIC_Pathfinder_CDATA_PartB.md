...
Despite decades of research, ageing lacks a mechanistic, causal explanation at the molecular level. The dominant framework — the 12 Hallmarks of Ageing (López-Otín et al., 2023, *Cell*) — catalogues *consequences* of ageing but does not identify a single initiating event. Telomere attrition (Blackburn, 2000) explains Hayflick-limited cell divisions but not post-mitotic cell ageing. Inflammaging (Franceschi et al., 2007) is a downstream consequence of immune dysregulation, not its root cause.
...
The downstream consequences form two independent tracks (Fig. 1 in supplementary figures):
...
| Tkemaladze J. "CDATA: Centriolar Damage Accumulation Theory of Ageing" | *Ann Rejuvenation Sci* | 2025 | **DOI: 10.65649/yx9sn772** — presents the full CDATA theoretical framework with computational validation and Ze Theory biomarker integration. |

Additionally, the PI has developed an independent biomarker for healthy ageing — **Ze Theory** (optimal biological velocity v* = 0.456) — which predicts HRV-derived longevity markers with r = 0.78 in historical genealogical cohorts (biblical patriarchal dataset, n=26) and constitutes the empirical backbone for CDATA clinical validation.
...
**Validated intervention library** (8 types including Yamanaka, Rapamycin/CR, Senolytics, NAD⁺, CentrosomeTransplant) with quantified healthspan extensions.
...
| Hallmarks of Ageing (López-Otín 2023) | Multiple, no hierarchy | Partial | ✅ | Multiple | No root cause |
| Telomere theory (Blackburn 2000) | Telomere shortening | ✅ | ❌ (post-mitotic cells have no divisions) | Telomerase activation | Cannot explain post-mitotic decline |
...
- **Biomarkers collected:**
  ...
  - Ze Theory HRV biomarker (5-min ECG, v* proximity score)
...
- **Secondary endpoints:** CAII vs. epigenetic clock acceleration; CEP164 vs. myeloid shift (monocyte/lymphocyte ratio); Ze Theory HRV vs. 2-year mortality
...
- **T3.2:** Implement patient-specific parameter fitting: given individual CAII + Ze + epigenetic age inputs, infer personalised DamageParams (personalised biological age = pBA)
- **T3.3:** Validate pBA against 2-year WP2 outcomes (Kaplan-Meier, AUROC for 2-year frailty progression)
- **T3.4:** Extend Cell-DT to include IGF-1 axis (insulin, leptin, ghrelin), asymmetric_cytoqc_module, and PTM columns in CSV exporter
...
**Intervention library (from Cell-DT, to be tested in WP4):**

| Intervention | CDATA mechanism targeted | Cell-DT predicted healthspan gain |
|-------------|-------------------------|----------------------------------|
| **CentrosomeTransplant** (young donor centriole transplantation) | Reset centriolar PTM burden to donor level | +18.3 years (simulation) |
...
- **Intervention response curves:** 8 validated interventions with realistic healthspan projections
...
#### 1.2.3 The Ze Theory Integration: Cross-Disciplinary Novelty

CDATA is uniquely integrated with **Ze Theory** — the PI's independent discovery that biological systems exhibit optimal flow velocity at v* = 0.456 (dimensionless), corresponding to entropy ≈ 1.68 nats. This predicts HRV-based longevity markers with documented r = 0.78 in a historical validation cohort. Ze Theory provides:

- A non-invasive clinical biomarker (5-minute ECG, applicable in primary care)
- A thermodynamic bridge linking cellular (centriole) and organismal (HRV) ageing readouts
- Independent validation of CDATA predictions at a different biological scale

This cross-scale integration — from nanometre centriolar PTMs to millisecond ECG intervals — is scientifically unprecedented and constitutes a major advance in systems biology of ageing.
...
**Ze Theory clinical tool (TRL 3 → 5):** Integration of Ze Theory HRV biomarker with CDATA clinical data will enable development of a validated wearable-compatible ageing index (NexRing, HealthWearable alignment).
...
**Preprints:** bioRxiv deposits at M12 (WP1 interim) and M24 (WP2 interim) ensure immediate community access.
...
- **Book:** *Medicine of Generations* (Jaba Tkemaladze) — translated into EN/KA/KZ — will serve as a public science communication vehicle, introducing CDATA concepts to general audience
...
| WP4 | Therapeutic PoC and Translation | [Applicant Institution: TBD] | M18 | M36 | €350,000 |
...
- T2.2 (M6–M18): Baseline assessment (n=288): CAII, CEP164, Ze HRV, epigenetic age, telomere length, clinical phenotyping (Fried, MMSE, grip)
...
- T3.2 (M12–M24): Implement personalised biological age (pBA) module: given individual CAII + Ze + epigenetic age → infer DamageParams via posterior sampling (Stan)
- T3.3 (M18–M30): Validate pBA against WP2 outcomes (AUROC for 2-year frailty progression; target AUROC > 0.75)
- T3.4 (M24–M36): Implement remaining roadmap items (asymmetric_cytoqc_module, IGF-1 axis, PTM CSV exporter, Ze v* = 0.456 entropy verification)
...
**WP4 — Therapeutic PoC and Translation** (Lead: [Applicant Institution: TBD], M18–M36, €350K)
...
*Key tasks:*
- T4.1 (M18–M24): Consortium Agreement finalised (M0 milestone); IP policy agreed
...
*Major deliverables:*
- D4.1 (M18): Consortium Agreement (Milestone M0)
...
| D4.1 | Consortium Agreement | WP4 | Admin | M18 |
...
| Clinical Research Nurse ×2 | [TBD institution] | — | 36×2=72 | — | — | **72** |
...
- **Infrastructure:** Access to high-performance computation (cloud HPC); Cell-DT development environment; drjaba.com AI-assisted clinical platform
- **Role in this project:** Scientific leadership, CDATA theoretical development, Cell-DT architecture, WP3/WP4 leadership

**[Applicant Institution: TBD]'s medical research direction** (formalised for this application): Integration of computational biology, longevity medicine, and Ze Theory biomarker research. Letter of support from [Applicant Institution: TBD] president confirming dedicated research space and administrative infrastructure.
...
- [Publication record and expertise to be confirmed upon reply to PI outreach; full CV to be inserted prior to submission]
...
- **[Applicant Institution: TBD]:** CDATA theory origination, Cell-DT computational development, Ze Theory clinical integration
...
**Letter of Intent for collaboration:** [Applicant Institution: TBD] — [TBD institution] MoU to be signed at M1 (Milestone M1).
...
| ECG Holter monitors × 20 (Ze Theory HRV) | WP2 | 18,000 | 5-minute HRV recordings for Ze Theory v* measurement |
...
**Subcontracting WP2:** Clinical Research Organization (CRO) services for data monitoring (€60,000) — justified by GCP ICH E6(R2) compliance requirement for independent monitoring in clinical study.
...
|---|------|-------------|--------|-----------|
| R1 | **[TBD institution] co-PI confirmation delayed** ([TBD]'s name still placeholder) | Medium | Medium | Alternative: recruit Prof. Nino Lomidze ([TBD institution] team, known Horizon track record). Timeline: final name by M1. |
| R2 | **CAII does not correlate with frailty** (primary endpoint fails) | Low-Medium | High | WP2 has 5 secondary endpoints (epigenetic clock, Ze HRV, telomere, immune markers, mortality); even negative result for CAII is publishable and scientifically informative. Cell-DT development (WP3) is independent of WP2 outcome. |
...
| R4 | **[Applicant Institution: TBD] PIC not granted** before M3 | Medium | High | **P0 critical action:** PIC registration initiated before project start. EU Funding Portal registration takes 2–4 weeks (not months); [TBD institution] PIC ([TBD PIC]) confirmed and will be used as administrative coordinator if [TBD] PIC is delayed beyond M1. |
...
1. Tkemaladze J, Lortkipanidze G. "Asymmetric segregation of the centrosome and its role in determining cell fate." *Mol Biol Rep.* 2023;50(3):2741-2748. **PMID: 36583780**

2. Tkemaladze J. "CDATA: Centriolar Damage Accumulation Theory of Ageing." *Ann Rejuvenation Sci.* 2025. **DOI: 10.65649/yx9sn772**

3. López-Otín C et al. "Hallmarks of aging: An expanding universe." *Cell.* 2023;186(2):243-278.

4. Blackburn EH. "Telomere states and cell fates." *Nature.* 2000;408:53-56.
...
7. Vasa M et al. "Increase in circulating endothelial progenitor cells by statin therapy in patients with stable coronary artery disease." *Circulation.* 2001;103(24):2885-90.
...
*Project acronym: CDATA | EIC Pathfinder Open 2026 | Deadline: 12.05.2026*
