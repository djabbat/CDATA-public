...
**Architecture:** ECS (Entity-Component-System) design using `hecs` crate; 14 simulation modules; ≥439 passing unit tests (+ new tests for modules added in WP3) (cargo test); parallelised via Rayon.
...
| Telomere theory (Blackburn 2000 [2]) | Telomere shortening | ✅ | ⚠️ Explains replicative senescence only; does not account for post-mitotic dysfunction† | Telomerase activation | Incomplete: CDATA complements telomere theory at the post-mitotic level |
| Inflammaging (Franceschi 2007) | Chronic inflammation | Partial | ✅ | Anti-inflammatory | Downstream, not causal |
| Free Radical theory (Harman 1956) | ROS accumulation | Partial | ✅ | Antioxidants | ROS is modulator, not initiator [11] |
...
**Experimental design:**
- **Cell models:** (a) *Protocol development:* HeLa cells (high-passage) for U-ExM and STED protocol optimisation only. (b) *Primary dose-response experiments:* IMR90 fibroblasts (low/high passage) and **primary human dermal fibroblasts (HDF)** isolated from young (25–35 y) vs aged (65–75 y) donors — the most physiologically relevant model. (c) *Ex vivo:* CD34⁺ EPCs (isolated from consented donors via MACS, Miltenyi protocol; 20 mL blood, Vasa 2001 protocol). HeLa is used solely for assay development due to its aneuploidy and atypical centrosomal apparatus; all quantitative conclusions will be drawn from IMR90 and HDF.
...
- **Key readouts:**
  - CEP164/CEP89/Ninein/CEP170 IF (Zeiss LSM 700 confocal at [TBD institution]; acquisition of a new Leica SP8 STED microscope (€120K) in WP1 for super-resolution appendage imaging. The existing Zeiss LSM 700 continues to be used for standard confocal imaging)
...
†*Note: Telomere shortening explains Hayflick-limited replicative senescence; CDATA extends this framework to post-mitotic cells (neurons, cardiomyocytes) where centriolar damage drives cilia dysfunction independently of division-linked telomere attrition.*
...
- **≥439 passing unit tests (+ new tests for modules added in WP3)** providing scientific reproducibility guarantees absent from all competing ageing models (ODE-based Hallmarks models typically provide no code or validation)
...
#### 1.2.3 Cross-Scale Integration and the Ze Theory HRV Hypothesis

CDATA bridges biological scales rarely connected in a single framework:
...
- **Cellular scale (μm):** Primary cilia length, spindle fidelity, senescence markers (SA-β-Gal, γ-H2AX) — directly downstream of CAII in the CDATA causal chain.
...
**Ze Theory HRV biomarker — an exploratory secondary hypothesis.** The PI's Ze Theory proposes that biological systems exhibit optimal information flow at a dimensionless velocity v* ≈ 0.456 (entropy ≈ 1.68 nats). Applied to cardiac dynamics, this predicts a candidate non-invasive biomarker: proximity of the HRV-derived flow velocity to v* (measured from standard 5-minute ECG). If validated, this would link nanometre-level centriolar ageing directly to millisecond cardiac rhythm complexity — an unprecedented cross-scale biomarker.

Ze Theory is treated as a **secondary, exploratory endpoint** in WP2. No prior cohort validation is assumed — WP2 provides the first rigorous prospective test in n=288 participants. A positive finding (HRV index correlated with frailty/mortality) would represent a novel screening tool; a negative finding remains informative. EIC Pathfinder's mandate for high-risk, high-gain science makes this an appropriate exploratory inclusion alongside the primary CAII validation.

This cross-scale integration — from nanometre PTMs to millisecond ECG intervals to clinical frailty — provides the mechanistic underpinning for Cell-DT's personalised biological age (pBA) module.
...
| 10 | Cell-DT vs. experimental validation | Pearson r > 0.5 (with biological replicates; n≥6) | WP3 technical report |
| **Therapeutic PoC** | | | |
| 11 | CDATA interventions tested in vitro | ≥ 3 interventions | WP4 lab report |
| 12 | Proof-of-mechanism confirmed for ≥ 1 CDATA intervention | Statistically significant reduction in SA-β-Gal or CAII score vs. control | WP4 lab report or preprint |
| **Translation** | | | |
...
| 16 | CAII ELISA compatibility with automated platform demonstrated | Feasibility report (Roche Cobas or equivalent) by M36 | Technical feasibility report |
...
**CAII clinical translation pathway:** The CAII ELISA assay (developed in WP1 T1.5) will be validated in WP2 on human blood samples. In M30–M36, CAII ELISA compatibility with an automated clinical chemistry analyser (target platform: Roche Cobas or equivalent) will be evaluated in collaboration with a diagnostic industry partner (identified via WP4 industry outreach). This step is designed to demonstrate the regulatory and technical feasibility of CAII as a routine clinical blood test, preparing the ground for an IVD (in vitro diagnostic) development pathway post-project.
...
*Equipment:* Leica SP8 STED acquisition (€120K, new instrument at [TBD institution] core facility; the existing Zeiss LSM 700 continues for standard confocal); Miltenyi MACS EasySep (€15K); standard cell culture consumables (€180K); reagents (€120K); open-access confocal time (€50K).
...
- T2.2 (M6–M18): Baseline assessment (n=288): CAII, CEP164, HRV index, epigenetic age, telomere length, clinical phenotyping (Fried, MMSE, grip). **All biomarker analyses performed blinded to clinical phenotype data; unblinding occurs only after the baseline dataset is locked (M18).**
- T2.3 (M18–M36): 2-year follow-up: repeat all biomarkers; incident frailty, hospitalisations, mortality recorded
- T2.4 (M24–M36): Statistical analysis: logistic regression (primary endpoint), correlation analysis (secondary endpoints), survival analysis. **Biomarker data are analysed by a biostatistician blinded to clinical outcomes; unblinding occurs only after the final dataset is locked (M30).**
...
*Key tasks:*
- T3.1 (M6–M18): Bayesian parameter estimation using WP1 dose-response data; replace Cell-DT defaults with experimentally derived kinetic rates. Rust Engineer position opened at M1 via international recruitment (open to EU and Horizon Associated Country candidates).
...
- New Leica SP8 STED microscope acquisition planned in WP1 budget (€120K). The existing Zeiss LSM 700 will continue to be used for standard confocal imaging.
...
**co-PI — [TBD], partner institution [TBD]
- Professor of Biomedical Engineering, partner institution [TBD]
- Member of Horizon Europe consortium (GA #101216703, 2025–2028); confirmed EU project management and international reporting experience
- **Role in CDATA:** oversight of WP1 in vitro experiments, WP2 cohort execution, and Leica SP8 STED imaging quality control; co-signatory on all [TBD institution] deliverables
- CV available upon request; to be submitted with Part A
...
*Equipment WP1: Leica SP8 STED acquisition €120K + cell culture equipment €50K + ELISA reader €30K

#### Table 3.3b: Purchase Costs — Major Equipment Items

| Item | WP | Cost (€) | Justification |
|------|----|----------|---------------|
| Leica SP8 STED microscope — new acquisition ([TBD institution] core) | WP1 | 120,000 | Nanoscale centriolar appendage imaging; U-ExM requires resolution < 100 nm. The Zeiss LSM 700 (existing) is used for standard confocal; Leica SP8 STED is a separate new instrument required for CEP164 nanotopology at < 100 nm resolution |
...
|---|------|-------------|--------|-----------|
| R1 | **[TBD institution] co-PI onboarding** | Low | Low | co-PI confirmed: [TBD] (partner institution [TBD]). CV and letter of support to be obtained before M1. |
...
| R3 | **Leica SP8 STED delivery delayed** (equipment supply chain) | Low | Medium | WP1 standard confocal (Zeiss LSM 700) used for months M1–M9; STED needed only from M10 for nanotopology. 3-month delay acceptable. Alternative: TSMU core facility confocal. |
...
*Document version: v7.0 — 2026-03-24*
*PI: Jaba Tkemaladze, MD — [Applicant Institution: TBD], Poti, Georgia*
*Contact: jaba@drjaba.com*
*Changes from v2.0 through v7.0: Ze Theory repositioned as secondary exploratory endpoint in WP2 (1.2.3) without reliance on prior unpublished datasets. Consortium Agreement corrected to M1. [Applicant Institution: TBD] description expanded. WP4 preparatory task T4.0 added (M6–M18). In silico caveats added. STED corrected to new Leica SP8 acquisition. WP2 blinding added. HDF primary fibroblasts added to WP1. KPIs adjusted (#10 r>0.5; #12 proof-of-mechanism; #16 CAII ELISA automated platform). Gladyshev 2014 reference added. International Rust Engineer recruitment added.*
