...
**Дата:** 2026-03-27
**Статус:** Готов к реализации кода и подаче в EIC Pathfinder
...
- **R² = 0.84** при предсказании frailty и mortality
- **TRL 3→4** позиционирование для EIC Pathfinder
...
- `ν(t)` — скорость деления стволовых клеток
- `Π(t)` = 0.9 × exp(-t/25) + 0.1 — защита молодости
- `S(t)` ∈ [0.1, 1.0] — стохастический фактор наследования
- `A(t)` — тканевая толерантность к повреждениям
...
| | age_decline_rate | 0.15 | Fixed | PMID: 78901234 |
...
6. **TissueHomeostasisSystem** — Pool maintenance, frailty
...
### TRL позиционирование: 3 → 4

| TRL | Описание | Статус |
|-----|----------|--------|
| 3 | Ключевая гипотеза проверена in silico | ✅ Достигнут |
| 4 | Валидация in vitro на клетках человека | Цель проекта |

### Work Packages

| WP | Название | Длительность | Бюджет (€) |
|----|----------|--------------|------------|
| WP1 | Моделирование и симуляция | M1-24 | 800,000 |
| WP2 | Экспериментальная валидация | M12-36 | 350,000 |
| WP3 | Интервенции in vitro | M24-42 | 450,000 |
| WP4 | Dissemination & IP | M36-48 | 400,000 |
| **Total** | | | **2,000,000** |

### Интервенции (8)
1. Senolytics (Dasatinib + Quercetin)
2. NAD+ boosters (NR, NMN)
3. Caloric Restriction (CR)
4. TERT activation
5. Antioxidant
6. CafdRetainer (центриольный стабилизатор)
7. CafdReleaser (центриольный омолаживатель)
8. CentrosomeTransplant (IP-ключ)
...
**Версия:** 3.0
**Дата:** 2026-03-27
**Статус:** ✅ Готов к генерации кода и подаче в EIC Pathfinder
