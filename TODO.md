...
**Статус:** частично (PTM-ставки как константы)
**Направление:** ввести температурозависимость и термодинамическое обоснование

- [ ] ThermodynamicState { local_temp: f32, entropy_production: f32, damage_rate_multiplier: f32 }
- [ ] damage_rate ∝ exp(-Eₐ / kʙ × T): при воспалении T→38-39°C → повреждения↑ экспоненциально
- [ ] Карбонилирование C=O как необратимый маркер (ΔG < 0 при любых условиях)
- [ ] Таблица Eₐ для каждого PTM: ацетилирование (низкий барьер) > окисление > агрегация
- [ ] Связь с InflammagingState: sasp_intensity → local_temp → damage_rate_multiplier
...
- [ ] **AppendageProteinState** — 4 отдельных f32 вместо одного ciliary_function
  - cep164_integrity: f32  // инициация IFT, Shh-сигналинг
  - cep89_integrity:  f32  // стабилизация дистального придатка
  - ninein_integrity: f32  // якорь MT к центриоли
  - cep170_integrity: f32  // удлинение придатка
- [ ] **ROSCascadeState** — 4 переменных вместо одного ros_level
  - superoxide: f32        // O₂⁻ (митохондрии)
  - hydrogen_peroxide: f32 // H₂O₂ (через SOD)
  - hydroxyl_radical: f32  // OH· (Фентон, самый разрушительный)
  - labile_iron: f32       // Fe²⁺/Fe³⁺ (катализатор Фентона)
- [ ] Разные чувствительности белков: OH· разрушает CEP164 > CEP89 > Ninein > CEP170
