...
- [x] **GUI: экспорт результатов** — кнопка "💾 Export CSV" в нижней панели после симуляции; CSV включает все 22 поля SimSnapshot (+ per_tissue_div_rate × 8, centrosomal_damage, biological_age) ✅ 2026-03-25
- [x] **GUI: все 11 вкладок иерархии** ✅ 2026-03-25
  - Society (+4): `SocialConditions` struct + sliders (loneliness, cohesion, SES, stress, activity, purpose); 3 пресета (Blue Zone / Urban / High Stress); damage_scale modifier ×[0.7..1.5]
  - Ecosphere (+5): добавлена AIM Integration панель — eco+social → combined damage_scale; статус бинарника; ссылка на Ze pipeline
  - Левая панель: Society ❌→✅, Ecosphere 🟡→✅
...
- [x] **AIMIntegration** ✅ 2026-03-25
  - `cdata_patient_sim` бинарник скомпилирован (`target/release/`)
  - `DamageParams::scaled(factor)` добавлен в `damage.rs` — масштабирует все молекулярные скорости
  - `HumanDevelopmentModule::set_damage_rates()` — публичный сеттер
  - `cdata_bridge.py` исправлен (путь к бинарнику) + добавлен `damage_scale_from_ze(ze_v, ze_state)`
  - `medical_system.py::show_aging_prediction()` читает Ze-HRV из SQLite → damage_scale
  - Протестировано: scale 0.6→99.9y, 1.0→75.4y, 3.0→30.1y, 5.0→18.9y ✅
...
- Структура вкладок:
  ...
  | Экосфера | +5 | Interventions: сенолитики, NAD+, CR |
