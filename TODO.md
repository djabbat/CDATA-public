...
## 🔴 Изменения v4.0 (2026-04-09)

| # | Задача | Приоритет | Статус |
|---|--------|-----------|--------|
| V4-1 | CONCEPT: Предсказание 3 заменено с roGFP на hTERT+гипоксия | Критический | ✅ Done |
| V4-2 | CONCEPT: Добавлен аргумент ¬R (логическое доказательство уникальности центриоли) | Критический | ✅ Done |
| V4-3 | CONCEPT: Добавлена фундаментальная форма `D(n)=1−e^{−rn}` | Высокий | ✅ Done |
| V4-4 | CONCEPT: Уточнён Dual-Clock — центриоль = универсальные часы, теломеры/ROS = модуляторы | Высокий | ✅ Done |
| V4-5 | CONCEPT: O₀=5% явно добавлен рядом с K_O2=0.2 | Средний | ✅ Done |
| V4-6 | **КОД:** Реализовать hTERT-интервенцию как отдельный endpoint в симуляторе | Высокий | ✅ Done (aging_engine/src/lib.rs) |
| V4-7 | **КОД:** Добавить `examples/htert_hypoxia_test.rs` — симуляция Эксперимента 3 | Высокий | ✅ Done (dual-model: аналитика + ODE) |
| V4-8 | **КОД:** Добавить `examples/not_r_argument.rs` — демонстрация насыщаемой кинетики D(n) | Средний | ✅ Done (мат. доказательство + таблица) |
| V4-9 | **КОД:** TissueState: добавить `current_o2_percent: f64` (default 21.0) + передача в MitochondrialSystem | Высокий | ✅ Done (уже был реализован) |
